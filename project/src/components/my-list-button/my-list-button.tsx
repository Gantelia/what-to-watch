...
function MyListButton({favoriteFilm}: MyListButtonProps): JSX.Element {
  ...
  const favoriteStatus = isFavorite(currentFavorite, favoriteFilm)
  ...
  return (
    ...
    >
      <svg viewBox="0 0 19 20" width="19" height="20">
        <use xlinkHref={`${isFavorite(currentFavorite, favoriteFilm)
           && isAuthorized(authorizationStatus) ? '#in-list' : '#add'}`}
        >
        </use>
      </svg>
      <span>My list</span>
